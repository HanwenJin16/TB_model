...
#We use Material class to store TB parameters
...
class Materials:
    ...
    def set_omega(self,omega):
        """
        This part is for quasi-static calculation and it is irrelevant here. 
        """
        self.omega=omega
        if not self.epsw:
            self.epsw=[]
            for cell in self.cells:
                ...
                elif (type(self.omega)==float or type(self.omega)==int):
                    f=get_default_material_data(symbol,"Permitivity_interpolator")
                    self.epsw.append(f([self.omega])[0])
                elif type(self.omega)==list:
                    f=get_default_material_data(symbol,"Permitivity_interpolator")
                    self.epsw.append(f(self.omega))


#We use Shape to create nanoparticles
class Shape:
    ...
    def compute_phi(self,positions,materials):
        """
        This is part of code that computes the quasi-static potential, this is irrelevant to the tight-binding construction. 
        """
        pass
...
#example shape
class Sphere(Shape):
    def __init__(self,radius=5,nregion=1,*args,**kwargs):
        super().__init__(nregion=1,**kwargs)
        self.radius=radius
        self.prefactor_list=None
